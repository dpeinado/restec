...
class RequestHandler(SocketServer.StreamRequestHandler):
    ...
    Call = dict(
            ...
            GET_LICENSE_LIST=(
                    lambda self, *args: self.get_license_list(*args))
            )
    ...
    def get_license_list(self):
        with self.CarsLock:
            myLicenses=self.Cars.keys()
        myLicenses.sort()
        return(True, myLicenses)
    
    def get_car_details(self, myLicense):
        with self.CarsLock:
            car = copy.copy(self.Cars.get(myLicense, None))
        if car is not None:
            return (True, (car.seats, car.mileage, car.owner))
        return (False, ("This license is not registered",))
        

    def change_mileage(self, myLicense, mileage):
        if mileage < 0:
            return (False, ("Cannot set a negative mileage",))
        with self.CarsLock:
            car = self.Cars.get(myLicense, None)
            if car is not None:
    ...
    def change_owner(self, myLicense, owner):
        if not owner:
    ...
    def new_registration(self, myLicense, seats, mileage, owner):
        if not myLicense:
        ...
        if not owner:
            return (False, ("Cannot set an empty owner",))
        with self.CarsLock:
            if myLicense not in self.Cars:
                self.Cars[myLicense] = Car(seats, mileage, owner)
                return (True, None)
...
def load(filename):
    if not os.path.exists(filename):
        ...
        for myLicense in ("1H1890C", "FHV449", "ABK3035", "215 MZN",
            ...
            cars[myLicense] = Car(seats, mileage, owner)
        return cars
        #return {}
    try:
