...
def display_children(cur, parent, level):
    ...
    if parent is None:
        msgStr=u"SELECT IdProject, IdProjectParent, Code, Description from projects where IdprojectParent is NULL"
    else:
        msgStr=u"SELECT IdProject, IdProjectParent, Code, Description from projects where IdprojectParent = {}".format(parent)
    cur.execute(msgStr)
    result = cur.fetchall()
    # display each child
    for row in result:
        msgStr =u"{0}: {1}".format(row[2],row[3])
        msg2 = ' '*level*5
        msg2=msg2+msgStr 
        print msg2
...
def rebuild_tree(cur, parent, left):
    ...
    msgStr=u"SELECT IdProject FROM projects where IdprojectParent = {}".format(parent)
    cur.execute(msgStr)   
    result = cur.fetchall()
    for row in result:
        right = rebuild_tree(cur,row[0],right) 
    msgStr=u"UPDATE projects SET lft = {0}, rgt = {1} WHERE IdProject = {2}".format(left,right,parent)
    cur.execute(msgStr)
...
def main():
    ...
    try:
        ...
        for pp in data:
            msgStr = u"INSERT INTO resources(IdResource,Name,Cost) VALUES({0},'{1}','{2}')".format(pp[0],pp[1],pp[2])
        ...
        data = curOld.fetchall()
        for pp in data:
            msgStr = u"INSERT INTO activities(IdActivity,Activity) VALUES({0},'{1}')".format(pp[0],pp[1])
            ok = curNew.execute(msgStr)
        newDB.commit()    
        ...
        misCodes={}
        msgStr = u"INSERT INTO PROJECTS(IdProject,Code,Description) VALUES({0},'{1}','{2}')".format(1,'','Proyectos')
        ok = curNew.execute(msgStr)
        for pp in data:
            idP = int(pp[0])+1
            misCodes[idP] = pp[1]
            msgStr = u"INSERT INTO PROJECTS(IdProject,IdProjectParent,Code,Description) VALUES({0},{1},'{2}','{3}')".format(idP,1,pp[1],pp[2])
            ok = curNew.execute(msgStr)
        newDB.commit()
        ...
        for entrada in data:
            ...
            msgStr = u"INSERT INTO PROJECTS(IdProject,IdProjectParent,Code,Description) VALUES({0},'{1}','{2}','{3}')".format(idP,indx,code,entrada[3])
            ok = curNew.execute(msgStr)
            idP += 1
        newDB.commit()
        ...
        for entrada in data:
            ...
            msgStr = u"""INSERT INTO ENTRIES(IdEntry,IdResource,IdProject,IdActivity,Tsec,creation_time,update_time) 
